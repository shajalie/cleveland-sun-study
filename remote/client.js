...
async function check(){try{const r=await fetch('/api/state');if(r.ok){update(await r.json());connect();}else{$('pair').hidden=false;$('app').hidden=true;$('pair-form').hidden=r.status===403;if(r.status===403)$('pair-error').textContent='Website access expired or was removed. Reload this page to sign in again, or contact the host.';}}catch{notice('The render computer is unreachable. Keep it running with the chosen website connection enabled.');setTimeout(check,3000);}}
...
document.addEventListener('keydown',e=>{if(/INPUT|SELECT|TEXTAREA/.test(e.target.tagName)||e.target.isContentEditable)return;const key=e.key.toLowerCase();if(['w','a','s','d'].includes(key)){e.preventDefault();held.add(key);send({type:'key',key,down:true});}});
