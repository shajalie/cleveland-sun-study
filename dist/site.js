(()=>{
 const panels=['outdoor','indoor','walk'];
 function select(name){if(!panels.includes(name))name='outdoor';for(const n of panels){document.getElementById('panel-'+n).hidden=n!==name;document.querySelector('[data-panel="'+n+'"]').setAttribute('aria-pressed',String(n===name));}for(const id of ['cs-play','ci-play']){const b=document.getElementById(id);if(b.textContent==='Pause')b.click();}if(name==='walk')window.ClevelandWalk?.activate();else window.ClevelandWalk?.pause();history.replaceState(null,'','#'+name);window.dispatchEvent(new Event('resize'));}
